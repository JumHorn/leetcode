...
int movesToChessboard(int** board, int boardSize, int* boardColSize)
{
	...
	if (rowSum != boardSize / 2 && rowSum != (boardSize + 1) / 2) return -1;
	if (colSum != boardSize / 2 && colSum != (boardSize + 1) / 2) return -1;
	if (boardSize % 2)
	{
		if (colSwap % 2) colSwap = boardSize - colSwap;
		if (rowSwap % 2) rowSwap = boardSize - rowSwap;
	}
	else
	{
