...
public:
	bool checkOverlap(int radius, int x_center, int y_center, int x1, int y1, int x2, int y2)
	{
		...
		if (x_center >= x1 && x_center <= x2 && y_center >= y1 && y_center <= y2)
			return true;
		return minDistanceToLine(x_center, y_center, x1, y1, x1, y2, radius) || minDistanceToLine(x_center, y_center, x1, y2, x2, y2, radius) || minDistanceToLine(x_center, y_center, x2, y2, x2, y1, radius) || minDistanceToLine(x_center, y_center, x2, y1, x1, y1, radius);
	}
	...
	bool minDistanceToLine(int x, int y, int x1, int y1, int x2, int y2, int radius)
	{
		...
		if (t < 0)
			...
			t = 1;
		double mindistance = ((x - x1) - t * (x2 - x1)) * ((x - x1) - t * (x2 - x1)) + ((y - y1) - t * (y2 - y1)) * ((y - y1) - t * (y2 - y1));
