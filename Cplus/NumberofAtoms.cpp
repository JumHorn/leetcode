...
public:
    string countOfAtoms(string formula) {
		map<string,int> atoms=countOfAtoms(formula,0,formula.length());
		string res;
		for(auto& n:atoms)
	...
	map<string,int> countOfAtoms(const string& formula,int start,int end)
	{
		...
		while(i<end)
		{
			if(formula[i]=='(')
			{
				...
				map<string,int> subatoms=countOfAtoms(formula,i+1,j);
				j++;
				i=j;
