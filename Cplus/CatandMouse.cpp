...
public:
	...
	{
		int N = graph.size();
		//state[node] : who wins in the end if we start from this node
		int state[50][50][3] = {0}; // state[mouse_location][cat_location][turn]
		...
		// enqueued : all nodes that we know who wins in the end. Nodes with DRAW state is not in this queue.
		queue<vector<int>> q;
		for (int i = 0; i < N; ++i)
		{
			for (int t = 1; t <= 2; ++t) //turn
			{
				state[0][i][t] = MOUSE_WIN; //The mouse wins if it is at the hole (position 0).
				q.push({0, i, t, MOUSE_WIN});
				if (i > 0)
				{
					state[i][i][t] = CAT_WIN; //The cat wins if mouse and cat are at the same location.
		...
		while (!q.empty())
		{
			int m = q.front()[0], c = q.front()[1], t = q.front()[2], s = q.front()[3]; //mouse_location, cat_location, turn, state
			q.pop();
			for (auto &parent : parents(graph, m, c, t)) // for every parent of this node (m, c, t) :
			{
				...
				if (state[m2][c2][t2] == DRAW)						// if we do not know who wins in this parent node
				{
					...
					else
					{
						...
						{
							state[m2][c2][t2] = t2 == MOUSE_TURN ? CAT_WIN : MOUSE_WIN;
							q.push({m2, c2, t2, state[m2][c2][t2]});
						}
					}
				}
			}
		...
		return state[1][2][MOUSE_TURN]; //The mouse is at location 1. The cat is at location 2. The mouse moves first.
	}
